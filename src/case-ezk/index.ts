// Entry point for the EZK case bundle. Imports the SCSS so MiniCssExtract emits
// dist/css/case-ezk.min.css alongside dist/js/case-ezk.min.js, and wires up
// scroll-driven animations (GSAP ScrollTrigger), per-image fade-in, and a top
// progress bar that tracks how many of the heavy mockup PNGs have downloaded.
...
function initHeroParallax(root: HTMLElement) {
  ...
  const layers: Array<{ el: HTMLElement; depth: number }> = [
    {
      el: root.querySelector<HTMLElement>('.case-ezk-hero__leaves--top-right')!,
      depth: -120,
    },
    {
      el: root.querySelector<HTMLElement>('.case-ezk-hero__leaves--bottom-left')!,
      depth: 80,
    },
    {
      el: root.querySelector<HTMLElement>('.case-ezk-hero__device')!,
      depth: -40,
    },
  ];
