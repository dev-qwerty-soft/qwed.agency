...
function bootCase() {
  const root = document.querySelector<HTMLElement>('.case-blue-jay');
  if (!root) return;
  ...
  if (!PREFERS_REDUCED_MOTION) {
    ...
    root
      .querySelectorAll<HTMLElement>(
        '.case-blue-jay__section, .case-blue-jay__image-section'
      )
...
function initProgressBar(root: HTMLElement) {
  ...
  bar.className = 'case-blue-jay__progress';
  bar.setAttribute('aria-hidden', 'true');
  const fill = document.createElement('div');
  fill.className = 'case-blue-jay__progress-fill';
