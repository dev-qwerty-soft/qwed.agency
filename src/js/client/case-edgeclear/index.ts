import '../../../scss/case/edgeclear/index.scss';
...
function bootCase() {
  const root = document.querySelector<HTMLElement>('.case-edgeclear');
  if (!root) return;

  initImageFadeIn(root);
  initCaseChrome(root, '#4bca86');

  if (!PREFERS_REDUCED_MOTION) {
    initSectionReveal(root);
  } else {
    root
      .querySelectorAll<HTMLElement>('.case-edgeclear__image-section')
      .forEach((el) => el.classList.add('is-revealed'));
  }
}

function initSectionReveal(root: HTMLElement) {
  const items = root.querySelectorAll<HTMLElement>('.case-edgeclear__image-section');
  items.forEach((el) => {
