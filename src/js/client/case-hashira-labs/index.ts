import '../../../scss/case/hashira-labs/index.scss';
import { initCaseChrome } from '../../utils/case-chrome';
import AOS from 'aos';

function bootCase() {
  const root = document.querySelector<HTMLElement>('.case-hashira-labs');
  if (!root) return;

  initImageFadeIn(root);
  initCaseChrome(root, '#e80004');
...
function initProgressBar(root: HTMLElement) {
  ...
  bar.className = 'case-hashira-labs__progress';
  bar.setAttribute('aria-hidden', 'true');
  const fill = document.createElement('div');
  fill.className = 'case-hashira-labs__progress-fill';
